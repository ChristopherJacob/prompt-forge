(function () {
  ...
  var vagueTerms = [
    'stuff', 'things', 'good', 'nice', 'better', 'quickly', 'asap', 'simple', 'robust', 'user friendly',
    'best', 'great', 'interesting', 'make it pop', 'somehow', 'various', 'etc'
  ];
  ...
  function analyzePrompt(rawText) {
    ...
    var vagueHits = vagueTerms.filter(function (term) {
    ...
    var breakdown = [
      {
        ...
        max: 15,
        points: cap((signals.role ? 4 : 0) + (signals.objective ? 6 : 0) + (signals.success ? 5 : 0), 15)
      },
      {
        key: 'contextInputs',
        label: 'Context and inputs',
        max: 15,
        points: cap((signals.context ? 6 : 0) + (signals.audience ? 4 : 0) + (signals.delimiter ? 5 : 0), 15)
      },
      {
        key: 'specificInstructions',
        label: 'Specific instructions',
        max: 15,
        points: cap((signals.instructions ? 7 : 0) + (signals.constraints ? 5 : 0) + (signals.positiveGuidance ? 3 : 0), 15)
      },
      {
        key: 'outputContract',
        label: 'Output contract',
        max: 15,
        points: cap((signals.outputFormat ? 6 : 0) + (signals.length ? 3 : 0) + (signals.tone ? 3 : 0) + (signals.structure ? 3 : 0), 15)
      },
      {
        key: 'examples',
        label: 'Examples',
        max: 10,
        points: cap((signals.examples ? 7 : 0) + (countMatches(lower, /input:|output:|good:|bad:|<example>/g) >= 2 ? 3 : 0), 10)
      },
      {
        key: 'reliability',
        label: 'Reliability and safety',
        max: 15,
        points: cap((signals.verification ? 4 : 0) + (signals.evidence ? 3 : 0) + (signals.uncertainty ? 3 : 0) + (signals.edgeCases ? 2 : 0) + (signals.safety ? 3 : 0), 15)
      },
      {
        key: 'structure',
        label: 'Prompt structure',
        max: 10,
        points: cap((signals.delimiter ? 5 : 0) + (/^(###|<role>|role:|you are|act as|objective:|task:)/i.test(text) ? 3 : 0) + (countMatches(text, /^###|^<[a-z]/gmi) >= 4 ? 2 : 0), 10)
      },
      {
        key: 'clarity',
        label: 'Clarity and precision',
        max: 5,
        points: cap((wordCount >= 60 ? 2 : 0) + (wordCount <= 1400 ? 1 : 0) + (vagueHits.length <= 2 ? 2 : 0), 5)
      }
    ];
    ...
    if (wordCount > 1400) suggestions.push('This prompt is long. Move reusable background to a separate knowledge base or trim duplicated instructions.');
    if (wordCount > 0 && wordCount < 60) suggestions.push('This prompt is short. Add context, success criteria, output format, and quality checks.');
  ...
  function summaryForScore(score, gates) {
    ...
    if (score >= 90 && missing.length === 0) return 'Ready to use. It has clear structure, context, output requirements, examples, and quality controls.';
    if (score >= 70) return missing.length ? 'Usable, but one or more best-practice gates still needs attention.' : 'Usable and well structured. Add examples or tighter quality controls to raise confidence.';
